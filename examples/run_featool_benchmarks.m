...
switch( i_solver )
  ...
  case 1   % FEATool
    ...
    while 1
      sline = fgetl( fid );
      if( any(strfind(sline,'---')) )
        break
      end
      it = it + 1;
    end
    while( ~any(strfind(sline,'t_tot')) )
      sline = fgetl( fid );
    end
  ...
  case 2   % FEniCS
    ...
    while 1
      ...
      if( any(strfind(sline,'t_solve')) )
        t_solv = str2num(sline(10:end));
      end
      if( any(strfind(sline,'Newton iteration')) )
        ix = find(sline == ':');
  ...
  case 3   % OpenFOAM
    ...
    while 1
      sline = fgetl( fid );
      if ~ischar(sline), break, end
      if( any(strfind(sline,'ExecutionTime')) )
  ...
  case 4   % SU2
    ...
    while 1
      ...
      if( any(strfind(sline,'SU2 solution time:')) )
